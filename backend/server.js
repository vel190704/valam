...
app.patch('/profile/risk', requireUser, async (req, res) => {
  try {
    ...
    if (!['low', 'medium', 'high'].includes(risk)) {
      return res.status(400).json({ error: 'Invalid risk value. Must be low, medium, or high.' })
    }
    const { error } = await supabaseAdmin
      .from('profiles').update({ risk_profile: risk }).eq('user_id', req.user.id)
...
app.get("/investments", requireUser, async (req, res) => {
  await processDueSips(req.user.id)
  const { data, error } = await supabaseAdmin
    .from("investments")
    .select("*")
    .eq("user_id", req.user.id)
...
app.delete('/sips/:id', requireUser, async (req, res) => {
  ...
  if (sipError) {
    return res.status(500).json({ error: sipError.message })
  }
